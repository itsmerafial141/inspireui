/// NOTE:
///
/// THIS WIDGET IS CLONED FROM THE PRELOAD_PAGE_VIEW PACKAGE.
...
class _PagePosition extends ScrollPositionWithSingleContext
  ...
  bool applyViewportDimension(double viewportDimension) {
    ...
    final page = (oldPixels == null || oldViewportDimensions == 0.0)
        ? _pageToUseOnStartup
        : getPageFromPixels(oldPixels, oldViewportDimensions!);
    final newPixels = page != null ? getPixelsFromPage(page) : null;
...
class _PreloadPageViewState extends State<PreloadPageView> {
  ...
  AxisDirection _getDirection(BuildContext context) {
    switch (widget.scrollDirection) {
      case Axis.horizontal:
        assert(debugCheckHasDirectionality(context), 'message');
  ...
  Widget build(BuildContext context) {
    ...
    final physics = widget.pageSnapping
